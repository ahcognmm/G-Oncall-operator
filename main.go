...
import (
	...
	"github.com/ahcogn/grafana-oncall-operator/controllers"
...
func init() {
	...
	utilruntime.Must(oncallv1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "ffe7b66a.ahcogn",
	...
	oncallUrl, err := url.Parse(os.Getenv("ONCALL_URL"))

	if err != nil {
		setupLog.Error(err, "unable to start manager cause ONCALL_URL error")
		os.Exit(1)
	}

	oncallToken := os.Getenv("ONCALL_TOKEN")

	if oncallToken == "" {
		setupLog.Error(err, "unable to start manager cause ONCALL_TOKEN not set")
		os.Exit(1)
	}

	config := oncallClient.Config{
		OncallUrl:  *oncallUrl,
		Credential: oncallToken,
	}
